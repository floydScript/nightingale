...
/// 合并 SoundAnalysis 产生的连续 "snoring" classifications 为单一事件。
/// SoundAnalysis 每 ~0.5-1 秒产一个 result；连续打呼会得到串，需要合并。
///
/// 策略：若新 detection 起点与当前 pending 事件的末尾之间间隔 ≤ `gapSeconds`，
/// 则延伸事件；否则开启新事件并把旧事件吐出。
struct SnoreDebouncer {
    ...
    init(gapSeconds: TimeInterval = 2.0) {
        self.gapSeconds = gapSeconds
    }

    /// 喂一个 detection。若并入当前 pending 返回 nil；若开启新事件返回上一个完成事件。
    mutating func feed(_ d: SnoreDetector.Detection) -> Merged? {
